...
def inventory():
    ...
    # Query spaces for this operator from fillurdetails_new
    inventory_cursor = db.fillurdetails_new.find({query_field: operator_phone})
    inventory = []
...
def show_agreement():
    ...
    # Check for signed agreement in fillurdetails_new
    signed_pdf_url = None
    signed_record = db.fillurdetails_new.find_one({'owner.phone': operator_phone, 'signed_pdf_url': {'$exists': True, '$ne': ''}})
    if signed_record:
        signed_pdf_url = signed_record['signed_pdf_url']

    if uploaded_pdfs:
        return render_template('show_agreement.html', uploaded_pdfs=uploaded_pdfs, signed_pdf_url=None)
    elif signed_pdf_url:
        return render_template('show_agreement.html', uploaded_pdfs=None, signed_pdf_url=signed_pdf_url)
    else:
        return render_template('show_agreement.html', uploaded_pdfs=None, signed_pdf_url=None)


@operators_bp.route('/upload_signed_agreement', methods=['POST'])
def upload_signed_agreement():
    ...
    # Save the file to static/uploads/operator_agreements/<phone>.pdf
    upload_dir = os.path.join(current_app.root_path, 'static', 'uploads', 'operator_agreements')
    os.makedirs(upload_dir, exist_ok=True)
    filename = f"{operator_phone}_signed_agreement.pdf"
    file_path = os.path.join(upload_dir, filename)
    file.save(file_path)

    # URL to serve the file
    signed_pdf_url = url_for('static', filename=f"uploads/operator_agreements/{filename}")

    # Update all fillurdetails_new records for this operator
    db.fillurdetails_new.update_many({'owner.phone': operator_phone}, {'$set': {'signed_pdf_url': signed_pdf_url}})

    flash('Signed agreement uploaded successfully.', 'success')
    return redirect(url_for('operators.show_agreement'))
...
@operators_bp.route('/update_availability/<space_id>', methods=['POST'])
def update_availability(space_id):
    if 'operator_phone' not in session:
        return jsonify({'status': 'failure', 'message': 'Unauthorized access'}), 401

    db = current_app.config['db']
    space = db.fillurdetails_new.find_one({'_id': ObjectId(space_id)})
    if not space:
        return jsonify({'status': 'failure', 'message': 'Property not found'}), 404

    inventory = space.get('inventory', [])
    form = request.form
    # Update inventory items
    for idx, item in enumerate(inventory):
        item['status'] = form.get(f'status_{idx}', item.get('status', 'available'))
        if 'count' in item:
            try:
                item['count'] = int(form.get(f'count_{idx}', item['count']))
            except:
                pass
        if 'price_per_seat' in item:
            try:
                item['price_per_seat'] = float(form.get(f'price_{idx}', item['price_per_seat']))
            except:
                pass
        # Room details
        if 'room_details' in item:
            for r_idx, room in enumerate(item['room_details']):
                try:
                    room['seating_capacity'] = int(form.get(f'room_seating_{idx}_{r_idx}', room['seating_capacity']))
                except:
                    pass
                try:
                    room['price'] = float(form.get(f'room_price_{idx}_{r_idx}', room['price']))
                except:
                    pass
                room['status'] = form.get(f'room_status_{idx}_{r_idx}', room.get('status', 'available'))

    db.fillurdetails_new.update_one({'_id': ObjectId(space_id)}, {'$set': {'inventory': inventory}})
